...
const CompanyPage: React.FC = () => {
  ...
  const fetchCompanyData = async () => {
    ...
    try {
      ...
      // First, fetch the user/customer data
  ...
  const getBreadcrumbItems = () => {
    // Check if we came from customers page
    if (location.pathname.includes('/customers/')) {
      return [
        { label: t('customers.title'), href: '/customers' },
        { label: customer?.company_name || t('company.companyDetails') }
      ];
    }
    
    // Default to business interest context
    return [
  ...
  const handleViewSolution = (solutionId: string) => {
    navigate(`/company/solutions/${solutionId}`);
  };
      ...
      <div className="min-h-screen bg-app-gradient">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="text-center py-12 bg-gradient-to-br from-gray-50 to-teal-50 rounded-xl shadow-lg">
            ...
            <h2 className="text-xl font-semibold text-gradient-primary mb-2">{t('company.companyNotFound')}</h2>
            <p className="text-gray-500 mb-6">{t('company.companyNotFoundDesc')}</p>
            <Button 
              variant="primary" 
              onClick={() => navigate(-1)}
    ...
    <div className="min-h-screen bg-app-gradient">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="mb-6 sm:mb-8 animate-fade-in-up">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-gradient-primary mt-4">{t('company.companyDetails')}</h1>
        </div>
